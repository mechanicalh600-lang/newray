...
import { Sliders, Save, Upload, Building, Clock, Lock, Users, ArrowRight, MessageSquare, Cog, HardDrive, Hash, ChevronDown, ChevronUp, Plus, Trash2, X, ImageIcon } from 'lucide-react';
import { supabase } from '../../supabaseClient';
import { useNavigate } from 'react-router-dom';
import { ClockTimePicker } from '../../components/ClockTimePicker';
import { MENU_ITEMS_CORE as MENU_ITEMS } from '../../config/menuItemsCore';
  ...
  const handleLoginBackgroundUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (file) {
          if (file.size > 2 * 1024 * 1024) {
              alert('حجم تصویر پس‌زمینه نباید بیشتر از ۲ مگابایت باشد.');
              return;
          }
          const reader = new FileReader();
          reader.onloadend = () => {
              setSettings(prev => ({ ...prev, login_background: reader.result as string }));
          };
          reader.readAsDataURL(file);
      }
      e.target.value = '';
          ...
          <form onSubmit={handleSave} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-1 space-y-6">
                  ...
                  <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                      <h3 className="font-bold text-gray-700 dark:text-gray-200 mb-4 flex items-center justify-center gap-2">
                          <ImageIcon className="w-5 h-5 text-primary"/> پس‌زمینه صفحه ورود
                      </h3>
                      <div
                          className="relative w-full h-36 mx-auto mb-3 bg-gray-100 dark:bg-gray-700 rounded-xl border border-gray-200 dark:border-gray-600 overflow-hidden group"
                          style={settings.login_background ? {
                      ...
                      >
                          {!settings.login_background && (
                              <div className="absolute inset-0 flex items-center justify-center text-gray-400 text-xs">
                                  بدون تصویر — از الگوی پیش‌فرض استفاده می‌شود
                              </div>
                          )}
                          <div
                      ...
                      <div className="flex items-center justify-center gap-3 text-sm">
                          ...
                          {settings.login_background && (
                              <button
                                  type="button"
                                  onClick={() => setSettings(prev => ({ ...prev, login_background: '' }))}
                      ...
                      <p className="text-[10px] text-gray-400 mt-2 text-center">حداکثر ۲ مگابایت — JPG یا PNG</p>
                  </div>
