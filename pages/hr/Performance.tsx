...
export const Performance: React.FC<Props> = ({ user }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const trackingCode = await fetchNextTrackingCode('PERF');
      const row = {
      ...
      const { data: inserted, error } = await supabase.from('performance_evaluations').insert(row).select('id').single();
      if (error) throw error;
      await startEntityWorkflow('PERFORMANCE', user, {
        entityId: inserted.id,
        trackingCode,
        title: `ارزیابی عملکرد: ${formData.personnelName}`,
        data: row,
      });
