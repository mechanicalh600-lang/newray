...
export const PurchaseRequests: React.FC<Props> = ({ user }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: inserted, error } = await supabase.from('purchase_requests').insert(payload).select('id').single();
      if (error) throw error;
      await startEntityWorkflow('PURCHASE', user, {
        entityId: inserted.id,
        trackingCode: formData.requestNumber.trim(),
        title: `درخواست خرید ${formData.requestNumber.trim()}`,
        data: payload,
      });
