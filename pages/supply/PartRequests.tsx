...
export const PartRequests: React.FC<Props> = ({ user }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: inserted, error } = await supabase.from('part_requests').insert(payload).select('id').single();
      if (error) throw error;
      await startEntityWorkflow('PART_REQUEST', user, {
        entityId: inserted.id,
        trackingCode,
        title: `درخواست قطعه ${trackingCode}`,
