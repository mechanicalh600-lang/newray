...
interface WorkOrdersProps {
  ...
  isViewOnly?: boolean;
  /** embed در کارتابل — بدون هدر و بدون ناوبری به صفحه مدیریت */
  embeddedInCartable?: boolean;
  onClose?: () => void;
}
...
export const WorkOrders: React.FC<WorkOrdersProps> = ({ initialData, onProcessComplete, isViewOnly = false, embeddedInCartable = false, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isExecutorMode && onProcessComplete) {
        ...
        try {
            ...
            const { data: inserted, error: dbError } = await supabase.from('work_orders').insert([dbPayload]).select('id').single();
            if (dbError) {
                console.error("Database Insert Error:", dbError);
                throw new Error("خطا در ذخیره دستور کار در دیتابیس.");
            }

            const cartableItem = await startWorkflow(
                'WORK_ORDER', 
                { ...fullData, work_type: formData.workType, ...dbPayload }, 
                user, 
                uniqueCode, 
          ...
          <div className="w-full max-w-full pt-8 pb-20 px-4">
              <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden animate-fadeIn">
                  ...
                  <div className="p-6 space-y-3">
                      <button onClick={() => (embeddedInCartable && onClose ? onClose() : navigate('/work-orders'))} className="w-full bg-white border border-gray-200 dark:bg-gray-700 py-3.5 rounded-xl flex items-center justify-center gap-2 font-medium hover:bg-gray-50 dark:hover:bg-gray-600">
                          <ListChecks className="w-5 h-5" /> {embeddedInCartable ? 'بازگشت به کارتابل' : 'بازگشت به لیست دستور کارها'}
                      </button>
                      {!embeddedInCartable && (
  ...
  return (
    <div className={`w-full max-w-full ${embeddedInCartable ? 'pb-4' : 'pb-24'}`}>
      
      {/* Header — در کارتابل هدر والد نمایش داده می‌شود */}
      {!embeddedInCartable && (
      <div className="flex justify-between items-center mb-6 sticky top-0 bg-gray-50 dark:bg-gray-900 z-20 py-2">
        ...
        <button type="button" onClick={() => (onClose ? onClose() : navigate('/work-orders'))} className="p-2 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-full"><X className="w-6 h-6"/></button>
      </div>
      )}
