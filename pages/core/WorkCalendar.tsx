...
import { getShamsiDate, getShiftRotation, jalaliToGregorian, toPersianDigits, parseShamsiDate } from '../../utils';
import { getHolidaysMapForYear } from '../../services/persianHolidays';
import { supabase } from '../../supabaseClient';
import { useNavigate } from 'react-router-dom';
import { WorkOrders } from '../maintenance/WorkOrders';
import { ShiftReportView } from '../reports/ShiftHandoverReport';
...
export const WorkCalendar: React.FC = () => {
  ...
  // تعطیلات رسمی شمسی برای سال جاری تقویم (تاریخ → عنوان مناسبت)
  useEffect(() => {
    let cancelled = false;
    getHolidaysMapForYear(currentYear).then(map => {
  ...
  return (
    <div className="w-full max-w-full space-y-6 pb-20">
              ...
              <div className="min-w-[800px]">
                  ...
                  <div className="grid grid-cols-7 auto-rows-fr">
                      ...
                      {daysArray.map(day => {
                          ...
                          const occasionTitle = holidayMap.get(dateStr);
                          const hoverTitle = [occasionTitle, isFriday ? 'جمعه' : null].filter(Boolean).join(' — ') || undefined;
                          ...
                          return (
                              ...
                              >
                                  <div className="flex justify-between items-center mb-2">
                                      <span className={`text-lg font-black ${isToday ? 'text-primary' : (isFriday || isHoliday ? 'text-red-500' : 'text-gray-700 dark:text-gray-200')}`}>
                                          {toPersianDigits(day)}
                                      </span>
                                      {isToday && <span className="text-[9px] bg-primary text-white px-1.5 py-0.5 rounded-md font-bold">امروز</span>}
                                      {isHoliday && !isToday && <span className="text-[9px] bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-400 px-1.5 py-0.5 rounded-md font-bold">تعطیل</span>}
                                  </div>
      ...
      {selectedDate && (() => {
          const parsed = parseShamsiDate(selectedDate);
          const isSelectedFriday = parsed ? (parsed.getDay() + 1) % 7 === 6 : false;
          const selectedOccasion = holidayMap.get(selectedDate) ?? (isSelectedFriday ? 'جمعه' : null);
          return (
              ...
              <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
                  ...
                  <div className="p-6 bg-gradient-to-r from-gray-900 to-gray-800 text-white flex justify-between items-start relative overflow-hidden">
                      ...
                      <div className="relative z-10">
                          ...
                          {selectedOccasion && (
                              <p className="text-sm mt-2 pt-2 border-t border-white/20 font-medium">
                                  مناسبت: {selectedOccasion}
                              </p>
                          )}
                      </div>
