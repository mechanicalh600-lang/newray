...
import { Factory, Save, ChevronRight, ChevronLeft, X } from 'lucide-react';
...
export const ProductionReport: React.FC<Props> = ({ user }) => {
  ...
  const handleDelete = async (ids: string[]) => {
      const { error } = await supabase.from('production_reports').delete().in('id', ids);
      if (error) {
          ...
          <div className="bg-white dark:bg-gray-800 flex-shrink-0 border-b border-gray-200 dark:border-gray-700 overflow-x-auto">
              <div className="flex min-w-max">
                  {PRODUCTION_TABS.map(tab => (
                      <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex-1 py-4 px-6 flex items-center justify-center gap-2 text-sm font-bold transition-all border-b-4 ${activeTab === tab.id ? 'border-primary text-primary bg-primary/5' : 'border-transparent text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'}`}>
          ...
          <form className="flex-1 min-h-0 flex flex-col" onSubmit={(e) => e.preventDefault()}>
            <div className="flex-1 min-h-0 overflow-y-auto px-1">
              <div className="py-4 space-y-6">
               ...
               {activeTab === 3 && <TabProduction reportDate={reportDate} />}
               {activeTab === 4 && <TabWaste reportDate={reportDate} />}
               {activeTab === 5 && <TabDowntime reportDate={reportDate} />}
               {activeTab === 6 && <TabAvailability reportDate={reportDate} />}
               {activeTab === 7 && <TabLogistics reportDate={reportDate} />}
              </div>
            </div>
               ...
               <div className="flex-shrink-0 flex items-center justify-between pt-4 pb-4 border-t dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
                  ...
                  {activeTab === PRODUCTION_TABS.length ? (
                      <button type="button" onClick={async () => {
                        ...
                        try {
                          const snapshot = await buildProductionReportSnapshot(reportDate);
                          const trackingCode = await fetchNextTrackingCode('PR-');
                          const parts = reportDate.split('/');
                          const year = parseInt(parts[0], 10);
                          const monthName = SHAMSI_MONTHS[(parseInt(parts[1], 10) || 0) - 1] as ShamsiMonthName;
                          const feedUsage = Number(budgetData.feedUsage || snapshot.feed.total || 0);
                          const prodUsage = Number(budgetData.prodUsage || snapshot.product.total || 0);
                          const feedDev = Number(budgetData.feedDevPercent || 100);
                          const prodDev = Number(budgetData.prodDevPercent || 100);

                          const payload = {
                            tracking_code: trackingCode,
                            report_date: reportDate,
                            total_production: prodUsage,
                            status: 'تکمیل شده',
                            full_data: {
                              reportDate,
                              budgetData,
                          ...
                          if (monthName && year) {
                            await syncProductionPlanUsage(year, monthName, feedUsage, prodUsage, feedDev, prodDev);
                          }
                          alert('گزارش جامع تولید ذخیره شد.');
                          await fetchReports();
                          setViewMode('LIST');
                        } catch (err: any) {
                          alert(`خطا: ${err?.message || 'ثبت ناموفق بود'}`);
                        }
