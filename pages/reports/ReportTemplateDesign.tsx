...
import { upsertReportDefinitionDraft } from '../../services/reportDefinitions';
...
export const ReportTemplateDesign: React.FC = () => {
  ...
  const moduleOptions = useMemo(
    () => reportOptions.map(o => ({ id: o.moduleId, label: o.title, slug: o.definitionSlug, path: o.path })),
    [reportOptions]
  );

  const getModuleLabel = (moduleId: string) => {
    const opt = findDesignOptionByModuleId(reportOptions, moduleId);
    return opt?.title || moduleOptions.find(m => m.id === moduleId)?.label || moduleId;
  };
  ...
  useEffect(() => {
    ...
    const onChanged = () => loadRegistry();
    loadRegistry();
    window.addEventListener('report-modules-changed', onChanged);
    window.addEventListener('report-definitions-changed', onChanged);
    return () => {
      window.removeEventListener('report-modules-changed', onChanged);
      window.removeEventListener('report-definitions-changed', onChanged);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    if (!opt) return;
    const activeTemplate = getActiveReportTemplateByModule(opt.moduleId);
    if (activeTemplate) {
      setTemplate(activeTemplate as any);
      setSelectedElementId(activeTemplate.elements?.[0]?.id || null);
    } else {
      const fallback = createDefaultTemplateForModule(opt.moduleId);
      fallback.title = `قالب ${opt.title}`;
      setTemplate(fallback);
      setSelectedElementId(fallback.elements?.[0]?.id || null);
    }
    setViewMode('DESIGN');
  ...
  }, [reportOptions.length, searchParams]);
  ...
  const handleSaveTemplate = async () => {
    const option = findDesignOptionByModuleId(reportOptions, template.targetModule);
    if (!option) {
      alert('گزارش معتبر انتخاب نشده. گزارش را از «طراحی گزارش» بسازید و دوباره انتخاب کنید.');
      return;
    }
    ...
    const defSlug = option.definitionSlug;
    try {
      const { data: existing } = await supabase
        .from('report_definitions')
        .select('id, template_schema, form_schema, list_schema, title, category')
        .eq('slug', defSlug)
        .maybeSingle();

      const nextTemplateSchema = {
        ...((existing?.template_schema as Record<string, unknown>) || {}),
        moduleId: option.moduleId,
        modulePath: option.path,
      ...
      if (existing?.id) {
        await supabase
          .from('report_definitions')
          .update({ template_schema: nextTemplateSchema })
          .eq('slug', defSlug);
      } else {
        await upsertReportDefinitionDraft({
          slug: defSlug,
          title: `${option.title} - فرم گزارش`,
          category: 'گزارشات',
          form_schema: { fields: [], tabs: [], groups: [] },
          list_schema: { columns: [] },
          template_schema: nextTemplateSchema,
        });
      }
      window.dispatchEvent(new Event('report-definitions-changed'));
    } catch {
    ...
    refreshTemplates();
    alert(`قالب ذخیره شد و به گزارش «${option.title}» متصل شد. نسخه ${saved.version} فعال شد.`);
  };
  ...
  const listColumns = useMemo(
    () => [
      {
        header: 'گزارش',
        accessor: (t: any) => getModuleLabel(t.targetModule || ''),
        sortKey: 'targetModule',
      },
      { header: 'عنوان قالب', accessor: (t: any) => t.title, sortKey: 'title' },
      {
        header: 'وضعیت فرم',
        accessor: (t: any) => {
          const status = linkStatuses.find(s => s.option.moduleId === t.targetModule);
          if (!status?.hasForm) {
            return <span className="text-orange-700 bg-orange-50 px-2 py-1 rounded text-xs">بدون فرم</span>;
          }
          if (status.formPublished) {
            return <span className="text-green-700 bg-green-50 px-2 py-1 rounded text-xs">فرم منتشرشده</span>;
          }
          return <span className="text-cyan-700 bg-cyan-50 px-2 py-1 rounded text-xs">پیش‌نویس فرم</span>;
        },
      },
    ...
    [linkStatuses, reportOptions]
  );

  const handleListAdd = () => {
    if (!reportOptions.length) {
      alert('ابتدا گزارش را از «طراحی گزارش» بسازید.');
      return;
    }
    const opt = reportOptions[0];
    const fallback = createDefaultTemplateForModule(opt.moduleId);
    fallback.title = `قالب ${opt.title}`;
    setTemplate(fallback);
    setSelectedElementId(fallback.elements?.[0]?.id || null);
               ...
               <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="flex flex-col gap-0.5 min-w-0">
                      <select value={template.targetModule} onChange={e => loadTemplateForModule(e.target.value)} className="text-sm bg-gray-100 dark:bg-gray-700 rounded px-2 py-1 border-none outline-none max-w-[220px]">
                          {moduleOptions.length ? moduleOptions.map(m => <option key={m.id} value={m.id}>{m.label}</option>) : (
                            <option value="">گزارشی یافت نشد</option>
                          )}
                      </select>
                      <p className="text-[10px] text-gray-500 truncate">
                        گزارش جدید از{' '}
                        <Link to="/report-design" className="text-primary hover:underline">طراحی گزارش</Link>
                        {findDesignOptionByModuleId(reportOptions, template.targetModule) ? (
                          <>
                            {' · '}
                            <Link
                              to={`/report-form-design?slug=${encodeURIComponent(findDesignOptionByModuleId(reportOptions, template.targetModule)!.slug)}`}
