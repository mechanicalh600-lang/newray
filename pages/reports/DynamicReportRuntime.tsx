...
import { ShiftReportFormContent, resolveFormRenderer } from '../../features/reports';
...
export const DynamicReportRuntime: React.FC<Props> = ({ user, slug: slugProp, initialDefinition }) => {
  ...
  const [formActiveTabId, setFormActiveTabId] = useState<string | null>(null);
  ...
  const formRenderer = resolveFormRenderer(formSchema, { preferShiftForControlRoom: isControlRoom });
  const useShiftStyle = formRenderer === 'shift_preset';

  useEffect(() => {
    if (mode === 'FORM' && isControlRoom && !useShiftStyle && tabs.length > 0) {
      setFormActiveTabId(tabs[0].id);
    }
  }, [mode, isControlRoom, useShiftStyle, tabs.length]);
  ...
  const validateForm = () => {
    ...
    for (const field of fields) {
      ...
      const isEmptyAttendance =
        ...
        (!value ||
          !Object.values((value as any).attendanceMap || {}).some(
      ...
      <div className="report-form w-full max-w-full h-full min-h-0 flex flex-col bg-gray-50 dark:bg-gray-900">
        ...
        <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
          <div className="flex-1 min-h-0 overflow-y-auto px-1">
            <div className={useShiftStyle ? 'py-4' : 'py-4'}>
        {useShiftStyle ? (
            <div className="border border-gray-200 dark:border-gray-600 rounded-xl overflow-hidden">
              <ShiftReportFormContent
                ...
                personnel={personnel}
                showNavButtons={true}
                showSubmitButton={isControlRoom}
                onSubmit={isControlRoom ? handleSave : undefined}
                initialValue={(() => {
                  const v = formValue;
                  return {
                    ...v,
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl border p-4">
              <DynamicFormRenderer
                ...
                personnel={personnel}
                onTabChange={isControlRoom ? (tabId) => setFormActiveTabId(tabId) : undefined}
              />
            </div>
          )}
            </div>
          </div>
          {!(isControlRoom && useShiftStyle) && (
            <div className="flex-shrink-0 pt-4 pb-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 flex items-center gap-2">
              {(!isControlRoom || (tabs.length > 0 && formActiveTabId === (tabs[tabs.length - 1]?.id ?? null))) && (
