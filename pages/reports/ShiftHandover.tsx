...
} from '../../features/reports/presets/shift/ShiftHandoverTypes';
import { 
    TabProduction, TabMills, TabHydrocyclones, TabDrumMagnets, 
    TabConcentrateFilters, TabThickeners, TabRecoveryFilters, TabDowntime, TabShiftNotes 
} from '../../features/reports/presets/shift/ShiftHandoverTabs';
...
export const ShiftHandover: React.FC<Props> = ({ user }) => {
  ...
  if (viewMode === 'FORM') {
    ...
    <div className="report-form w-full max-w-full h-full min-h-0 flex flex-col bg-gray-50 dark:bg-gray-900">
      ...
      <form className="flex-1 min-h-0 flex flex-col" onSubmit={(e) => { e.preventDefault(); setIsConfirmModalOpen(true); }}>
        <div className="flex-1 min-h-0 overflow-y-auto px-1">
          ...
          {activeTab === 9 && <TabDowntime downtime={downtime} setDowntime={setDowntime} isReadOnly={isReadOnly} handleDowntimeChange={handleDowntimeChange} handleVoiceInputMultiline={handleVoiceInputMultiline} shiftDuration={shiftInfo.shiftDuration} />}
          {activeTab === 10 && <TabShiftNotes downtime={downtime} setDowntime={setDowntime} footer={footer} setFooter={setFooter} pumps={pumps} setPumps={setPumps} isReadOnly={isReadOnly} handleDynamicListChange={handleDynamicListChange} addDynamicRecord={addDynamicRecord} removeDynamicRecord={removeDynamicRecord} handleVoiceInputMultiline={handleVoiceInputMultiline} handleVoiceInputDynamic={handleVoiceInputDynamic} />}
          </div>
        </div>
