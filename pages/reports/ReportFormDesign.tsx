...
import { ShiftReportFormContent, SHIFT_TABS_PRESET } from '../../features/reports';
import {
  findDesignOptionBySlug,
  loadAllReportDesignLinkStatuses,
  loadReportDesignOptions,
  ReportDesignLinkStatus,
  ReportDesignOption,
} from '../../services/reportDesignRegistry';
import { normalizeReportModulePath } from '../../services/reportModules';
import {
  ReportDefinition,
  ReportFieldSchema,
  ReportFormGroup,
...
export { SHIFT_TABS_PRESET } from '../../features/reports/presets/shift/tabsPreset';
...
export const ReportFormDesign: React.FC<ReportFormDesignProps> = ({ user }) => {
  ...
  const [reportOptions, setReportOptions] = useState<ReportDesignOption[]>([]);
  const [linkStatuses, setLinkStatuses] = useState<ReportDesignLinkStatus[]>([]);
  ...
  useEffect(() => {
    ...
    loadDefs();
    loadOptions();
    window.addEventListener('report-modules-changed', loadOptions);
    window.addEventListener('report-definitions-changed', loadOptions);
    return () => {
      window.removeEventListener('report-modules-changed', loadOptions);
      window.removeEventListener('report-definitions-changed', loadOptions);
    };
  }, []);

  useEffect(() => {
    const slugParam = searchParams.get('slug');
    if (!slugParam || !reportOptions.length || viewMode !== 'LIST') return;
    const opt = findDesignOptionBySlug(reportOptions, slugParam);
    const row = defs.find(d => d.slug === (opt?.definitionSlug || slugParam));
    if (row) {
      handleEdit(row);
    } else if (opt) {
      resetForm();
      setSelectedReportSlug(opt.slug);
      setTitle(`${opt.title} - فرم گزارش`);
      setSlug(opt.definitionSlug);
      setIsMetaOpen(true);
      setViewMode('FORM');
    }
    setSearchParams({}, { replace: true });
  // eslint-disable-next-line react-hooks/exhaustive-deps -- deep link once
  }, [defs.length, reportOptions.length, searchParams]);
  ...
  const selectedReport = useMemo(
    () => findDesignOptionBySlug(reportOptions, selectedReportSlug || slug),
    [reportOptions, selectedReportSlug, slug]
  );
  ...
  const columns = useMemo(
    () => [
      {
        ...
        accessor: (i: ReportDefinition) => {
          const opt = findDesignOptionBySlug(reportOptions, i.slug) ||
            reportOptions.find(o => (i.template_schema as any)?.modulePath === o.path);
          return opt?.title || i.category || '—';
        },
      },
      { header: 'عنوان فرم', accessor: (i: ReportDefinition) => i.title, sortKey: 'title' },
      ...
      { header: 'نسخه منتشرشده', accessor: (i: ReportDefinition) => i.published_version || 0, sortKey: 'published_version' },
      {
        header: 'قالب چاپ',
        accessor: (i: ReportDefinition) => {
          const status = linkStatuses.find(s => s.option.definitionSlug === i.slug);
          if (!status?.hasTemplate) {
            return <span className="text-orange-700 dark:text-orange-400 bg-orange-50 dark:bg-orange-900/30 px-2 py-1 rounded text-xs">بدون قالب</span>;
          }
          return <span className="text-green-700 dark:text-green-400 bg-green-50 dark:bg-green-900/30 px-2 py-1 rounded text-xs">متصل</span>;
        },
      },
      {
        header: 'وضعیت',
    ...
    [reportOptions, linkStatuses]
  );

  const saveDraft = async () => {
    try {
      if (!selectedReport && !editingId) {
        alert('ابتدا گزارش مورد نظر را انتخاب کنید. گزارش جدید را از «طراحی گزارش» بسازید.');
        return;
      }
      ...
      const result = await upsertReportDefinitionDraft({
        ...
        template_schema: {
          moduleId: selectedReport?.moduleId || (selectedReportSlug ? selectedReportSlug.replace(/-/g, '') : slug.trim()),
          modulePath: selectedReport?.path || normalizeReportModulePath(slug),
        },
      } as any);
  ...
  const handleEdit = (row: ReportDefinition) => {
    ...
    const opt =
      findDesignOptionBySlug(reportOptions, row.slug) ||
      reportOptions.find(o => (row.template_schema as any)?.modulePath === o.path);
    setSelectedReportSlug(opt?.slug || row.slug);
    setIsMetaOpen(true);
      ...
      <div className="h-[calc(100vh-64px)] flex flex-col bg-gray-100 dark:bg-gray-900 overflow-hidden">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border p-4 space-y-3">
          <div className="flex items-center justify-between">
            <SectionHeader index={1} title="عنوان و مشخصات فرم" help="گزارش را از لیست «طراحی گزارش» انتخاب کنید. slug و مسیر ماژول خودکار از همان گزارش پر می‌شوند." />
            <button
              type="button"
          ...
          <div className="space-y-3">
            <div className="space-y-1">
              <div className="text-xs font-bold text-gray-500 flex items-center gap-1">
                انتخاب گزارش <span className="text-red-500">*</span>
                <HelpHint text="گزارشی که در «طراحی گزارش» ساخته‌اید. فرم به همان گزارش در منو و دیتابیس متصل می‌شود. گزارش جدید را ابتدا از طراحی گزارش بسازید." />
              </div>
              <select
                ...
                onChange={e => {
                  ...
                  setSelectedReportSlug(next);
                  const opt = findDesignOptionBySlug(reportOptions, next);
                  if (opt && !editingId) {
                    setTitle(`${opt.title} - فرم گزارش`);
                    setSlug(opt.definitionSlug);
                    setCategory('گزارشات');
              ...
              <p className="text-[11px] text-gray-500">
                گزارش جدید؟{' '}
                <Link to="/report-design" className="text-primary hover:underline font-medium">
                  طراحی گزارش
                </Link>
              </p>
            </div>
          </div>
          {(selectedReportSlug || editingId) && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-2 border-t">
              <div className="space-y-1">
                <div className="text-xs font-bold text-gray-500 flex items-center gap-1">
                  نام فرم <span className="text-red-500">*</span>
                  <HelpHint text="نام نمایشی فرم که در منوی گزارش‌ها و عنوان بالای صفحه ثبت دیده می‌شود." />
                </div>
              ...
              <div className="space-y-1">
                <div className="text-xs font-bold text-gray-500 flex items-center gap-1">
                  ...
                  <HelpHint text="شناسه یکتا برای اتصال به قالب چاپ و ذخیره داده‌ها. از «طراحی گزارش» تعیین می‌شود." />
                </div>
                <input className="w-full p-2 border rounded-lg bg-gray-50 dark:bg-gray-900/40 font-mono" placeholder="مثال: shift-report" value={slug} readOnly={!!selectedReport} onChange={e => setSlug(e.target.value)} />
              </div>
              <div className="space-y-1">
                <div className="text-xs font-bold text-gray-500 flex items-center gap-1">کلید ماژول</div>
                <input
                  className="w-full p-2 border rounded-lg bg-gray-50 dark:bg-gray-900/40 font-mono"
                  value={selectedReport?.moduleId || ''}
                  placeholder="خودکار از گزارش"
                  readOnly
                />
              </div>
              <div className="space-y-1">
                <div className="text-xs font-bold text-gray-500 flex items-center gap-1">مسیر ماژول</div>
                <input
                  className="w-full p-2 border rounded-lg bg-gray-50 dark:bg-gray-900/40 font-mono"
                  ...
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      ...
                      <input
                        ...
                        min={1}
                        max={tabs.length}
                        className="input-number-rtl-center w-12 p-1 text-xs border rounded bg-white dark:bg-gray-700"
                        value={idx + 1}
                        onChange={(e) => {
                  ...
                  <FieldCategory title="اطلاعات اصلی" color="#2563eb">
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <div className="space-y-1">
                        ...
                        <select className="w-full p-2 border rounded bg-white dark:bg-gray-700" value={f.width || 2} onChange={e => setFields(prev => prev.map((x, i) => (i === idx ? { ...x, width: Number(e.target.value) as 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 } : x)))}>
                          {FIELD_WIDTH_OPTIONS.map(({ value, label }) => (
                    ...
                    {f.type === 'number' && (
                      <>
                        <div className="space-y-1">
                          ...
                          <input type="number" className="input-number-rtl-center w-full p-2 border rounded bg-white dark:bg-gray-700" placeholder="min" value={f.validation?.min ?? ''} onChange={e => setFields(prev => prev.map((x, i) => (i === idx ? { ...x, validation: { ...(x.validation || {}), min: e.target.value === '' ? undefined : Number(e.target.value) } } : x)))} />
                        </div>
                        <div className="space-y-1">
                          <div className="text-[11px] text-gray-500">حداکثر عدد</div>
                          <input type="number" className="input-number-rtl-center w-full p-2 border rounded bg-white dark:bg-gray-700" placeholder="max" value={f.validation?.max ?? ''} onChange={e => setFields(prev => prev.map((x, i) => (i === idx ? { ...x, validation: { ...(x.validation || {}), max: e.target.value === '' ? undefined : Number(e.target.value) } } : x)))} />
                        </div>
                      </>
                    )}
                    <div className="space-y-1 col-span-2">
                  ...
                  <FieldCategory title="شرطی و پیشرفته" color="#64748b" help="نمایش شرطی: فیلد فقط وقتی دیده می‌شود که فیلد مرجع (مثلاً shift_type) دقیقاً مقدار مورد انتظار (مثلاً Day1) را داشته باشد. هر دو فیلد را پر کن.\n\nلیست انتخابی: گزینه‌ها را با فرمت label:value هر خط وارد کن یا از «بارگذاری از جدول» استفاده کن.\n\nجفت زمان/ماتریس: فیلد هدف برای برابری مجموع، کلید فیلدی است که جمع مقادیر باید با آن برابر باشد. برچسب‌های مدت کارکرد و توقف برای جفت زمان قابل تنظیم است.">
                    ...
                    {f.type === 'repeatable_list' && (
                      <>
                        ...
                        <input
                          type="number"
                          className="input-number-rtl-center w-full p-2 border rounded bg-white dark:bg-gray-700"
