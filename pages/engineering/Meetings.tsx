...
export const Meetings: React.FC<Props> = ({ user }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: inserted, error } = await supabase.from('meeting_minutes').insert(row).select('id').single();
      if (error) throw error;
      await startEntityWorkflow('MEETING', user, {
        entityId: inserted.id,
        trackingCode,
        title: `صورتجلسه: ${formData.subject.trim()}`,
        data: row,
      });
