...
export const Projects: React.FC<Props> = ({ user }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: any = { title: formData.title.trim(), manager_id: formData.managerId, manager_name: manager?.full_name || '', start_date: formData.startDate || null, end_date: formData.endDate || null, budget: budgetNum, description: formData.description?.trim() || null, objectives: objectives.map(o => ({ id: o.id, text: o.text })), wbs: wbsItems.map(i => ({ id: i.id, code: i.code, title: i.title, parentId: i.parentId, order: i.order, percent: i.percent != null ? i.percent : undefined })), status: existing?.status || 'PLANNED', progress: existing?.progress ?? 0 };
      if (editingId) {
        await supabase.from('projects').update(payload).eq('id', editingId);
        alert('پروژه با موفقیت ویرایش شد.');
      } else {
        const { data: inserted, error } = await supabase.from('projects').insert([{ ...payload, tracking_code: code }]).select('id').single();
        if (error) throw error;
        await startEntityWorkflow('PROJECT', user, {
          entityId: inserted.id,
          trackingCode: code,
          title: `پروژه: ${formData.title.trim()}`,
          data: { ...payload, tracking_code: code },
        });
