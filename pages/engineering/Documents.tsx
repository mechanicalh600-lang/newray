...
export const Documents: React.FC = () => {
  ...
  const handleAdd = async (e: React.FormEvent) => {
      ...
      try {
          ...
          const trackingCode = await fetchNextTrackingCode('DOC');
          const row = {
              ...
              requester_name: user?.fullName || null,
          };
          const { data: inserted, error } = await supabase.from('technical_documents').insert(row).select('id').single();
          if (error) throw error;
          if (user) {
            await startEntityWorkflow('TECH_DOC', user, {
              entityId: inserted.id,
              trackingCode,
              title: `سند فنی: ${newDoc.name.trim()}`,
              data: row,
            });
          }
          ...
          if (msg.includes('Bucket not found') || msg.includes('bucket') || msg.includes('Storage')) {
              ...
              const { data: inserted, error: insErr } = await supabase.from('technical_documents').insert(row).select('id').single();
