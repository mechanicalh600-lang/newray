...
export const Suggestions: React.FC<Props> = ({ user }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: inserted, error } = await supabase.from('technical_suggestions').insert(row).select('id').single();
      if (error) throw error;
      await startEntityWorkflow('SUGGESTION', user, {
        entityId: inserted.id,
        trackingCode,
        title: `پیشنهاد فنی ${trackingCode}`,
        data: row,
      });
      ...
      if (msg.includes('Bucket not found') || msg.includes('bucket') || msg.includes('Storage')) {
        ...
        const { data: inserted, error: insErr } = await supabase.from('technical_suggestions').insert(row).select('id').single();
        if (insErr) {
