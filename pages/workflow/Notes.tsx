...
import { requestNoteNotificationPermission } from '../../utils/noteReminderAlert';
...
export const Notes: React.FC<Props> = ({ user }) => {
  ...
  useEffect(() => {
    ...
    if (!highlightId || loading || notes.length === 0) return;
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: Record<string, unknown> = {
        ...
        tags: normalizeNoteTags(editingNote.tags),
      ...
      if (hasReminder) {
        const perm = await requestNoteNotificationPermission();
        if (perm === 'denied') {
          alert('برای دریافت اعلان در زمان یادآوری، مجوز نوتیفیکیشن مرورگر را از تنظیمات سایت فعال کنید.');
        }
      }
      ...
      alert('خطا در ذخیره یادداشت: ' + formatDbError(e));
    }
  };
          ...
          <form onSubmit={handleSave} className="bg-white dark:bg-gray-800 w-full max-w-md rounded-2xl shadow-2xl p-6 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">
                در زمان تعیین‌شده، آلارم صوتی و نوتیفیکیشن مرورگر نمایش داده می‌شود.
              </p>
            </div>
