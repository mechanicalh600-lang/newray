...
interface Props {
  user: User;
  /** فقط پیام‌های دریافتی — بدون ارسال/ارسال‌شده */
  inboxOnly?: boolean;
  /** embed داخل کارتابل — بدون sidebar و padding صفحه */
  embedded?: boolean;
  /** تغییر تعداد نخوانده (مثلاً -1) */
  onUnreadChange?: (delta: number) => void;
}

export const Messages: React.FC<Props> = ({ user, inboxOnly = false, embedded = false, onUnreadChange }) => {
  const [activeTab, setActiveTab] = useState<'INBOX' | 'SENT' | 'COMPOSE'>(inboxOnly ? 'INBOX' : 'INBOX');
  ...
  const handleMarkReadAndClose = async () => {
      ...
      if (!selectedMsg.readBy.includes(user.id)) {
          setInboxMsgs(prev => prev.map(m => m.id === selectedMsg.id ? { ...m, readBy: [...m.readBy, user.id] } : m));
          await markMessageReadInDb(selectedMsg, user.id);
          onUnreadChange?.(-1);
      }
      setSelectedMsg(null);
  };
  ...
  const rootClass = embedded
    ...
    : 'w-full max-w-full h-[calc(100vh-100px)] flex flex-col md:flex-row gap-6 p-4';

  return (
    <div className={rootClass}>
        
        {/* Sidebar — مخفی در حالت embed کارتابل */}
        {!inboxOnly && (
        ...
        {/* Main Content */}
        <div className={`flex-1 bg-white dark:bg-gray-800 ${embedded ? '' : 'rounded-xl shadow-sm border border-gray-100 dark:border-gray-700'} overflow-hidden flex flex-col`}>
            
            {activeTab === 'COMPOSE' && !inboxOnly && (
            ...
            {(inboxOnly || activeTab === 'INBOX' || activeTab === 'SENT') && (
                        ...
                        <div className="flex-1 flex flex-col h-full bg-gray-50 dark:bg-gray-900/50">
                            <div className="p-6 bg-white dark:bg-gray-800 shadow-sm border-b dark:border-gray-700">
                                <div className="flex justify-between items-start mb-4 gap-3">
                                    <h2 className="text-xl font-bold flex-1">{selectedMsg.subject}</h2>
