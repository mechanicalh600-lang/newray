...
import { MatrixCellInput } from '../../../../components/MatrixCellInput';
import {
  getProductionTimes,
  TABS,
  parseTimeToMinutes,
...
export interface CustomBoxField {
  ...
  color?: string;
  helpText?: string;
  validation?: { min?: number; max?: number; totalMustEqualField?: string; dependsOn?: { field: string; equals: any } };
...
export const ShiftReportFormContent: React.FC<ShiftReportFormContentProps> = ({
  ...
  const currentTabId = designTabs && designTabs.length >= 9 && designTabs[activeTab - 1]
    ...
    : `tab-${activeTab}`;
  const setActiveTab = (v: number | ((prev: number) => number)) => {
    const next = typeof v === 'function' ? v(activeTab) : v;
    if (controlledActiveTab == null) setInternalTab(next);
  ...
  const [downtime, setDowntime] = useState(() => {
    ...
    const fromInit = initialValue?.downtime ?? {};
    return {
      ...base,
      ...fromInit,
      lineA: { ...base.lineA, ...(fromInit as any).lineA },
      lineB: { ...base.lineB, ...(fromInit as any).lineB },
      plant: { ...base.plant, ...(fromInit as any).plant },
    };
  });
                  ...
                  <div key={sectionId} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-[repeat(24,minmax(0,1fr))] gap-3">
                      {sectionFields.filter(f => f.type !== 'container').filter(field => {
                        ...
                        const spanStyle = getFieldSpanStyle(field.width, 4);
                          ...
                          >
                            ...
                            {field.type === 'date' ? (
                              ...
                              (() => {
                                ...
                                const personnelNormList = personnelNorm;
